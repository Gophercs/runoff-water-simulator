...
### The lidar relay (built in)

Wales publishes its lidar as one Wales-wide Cloud Optimized GeoTIFF, which Runoff reads in small pieces (just your area, and the file's built-in overviews for the wide preview). The storage it sits on allows those partial reads but doesn't send the header browsers need to use them from another site, so Runoff reads it through a relay. `functions/relay.js` is a Cloudflare Pages Function: it deploys with the site and answers at `/relay`, on the same address as the app, so no cross-site rules apply. (`relay/worker.js` is the same thing as a standalone Worker if you host elsewhere.) The national services do the same with their coverage services, so Runoff tries each service directly first and falls back to the relay when the browser won't let it read the reply. It relays only the national lidar services (Environment Agency, DataMapWales, Scottish Remote Sensing Portal), so it can't be used as a general proxy. When a relay is set, all lidar requests go through it, which also means Cloudflare's Worker metrics show you how those services are behaving: request counts, errors and status codes.

On Cloudflare Pages it needs no setup: `functions/relay.js` deploys automatically and the build points at `/relay`. To use a standalone Worker instead, deploy `relay/worker.js` and set the `RUNOFF_RELAY` environment variable to its address. To turn the relay off, set `RUNOFF_RELAY` to an empty value. The free Workers plan allows 100,000 requests a day; loading an area takes a few dozen.
